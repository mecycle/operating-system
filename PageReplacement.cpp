...
public:
	...
	int judge_change(){
		int fr = frame[0].frequency;
		vector<int> place;
		vector<int> pla;
		for (int i = 0; i < frame.size(); ++i)
		{
			if (frame[i].frequency <fr)
			{
				fr = frame[i].frequency;
			}
		}
		for (int i = 0; i < frame.size(); ++i)
		{
		...
		if (place.size()==1)
		{
			...
			string p = frame[place[0]].refer;
		
			for (int i = 0; i < place.size(); ++i)
			{
				if (frame[place[i]].refer.compare(p)<0)
				{
					p = frame[place[i]].refer;
			...
			if (pla.size()==1)
			{
				...
				int t = frame[pla[0]].time;
				int pl = pla[0];
				for (int i = 0; i < pla.size(); ++i)
				{
					if (frame[pla[i]].time<t)
					{
						t = frame[pla[i]].time;
						pl = pla[i];
	...
	bool chec(vector<page> com, page s){

		for (int j = 0; j < com.size(); ++j)
		{

			if (com[j].pageNumber == s.pageNumber)
			{
				return true;
			}
		}
	...
	void judge_ws(){
		...
		for (int i = 0; i < counter-1; i++)
		{
			...
			if (!chec(com,win[win.size()-i-2]))
			{
	
				win[win.size()-i-2].frequency++;
				com.push_back(win[win.size()-i-2]);
			}else{
				for (int j = 0; j <com.size(); ++j)
				{
					if (com[j].pageNumber == win[win.size()-i].pageNumber)
					{
						com[j].frequency++;
					}
				}
		...
		for (int i = 0; i < frame.size(); ++i)
		{
			for (int j = 0; j < com.size(); ++j)
			{
				if (frame[i].pageNumber == com[j].pageNumber)
				{
					frame[i].frequency = com[j].frequency;
				}
			}
		}
...
int main(int argc, char const *argv[])
{
	...
	if (file_type=="FIFO")
	{
		...
		m.simply();
		m.print_output();
	}else if(file_type=="WSARB"){
		a_register = atoi(argv[5]);
		shift_time = atoi(argv[6]);
		counter = atoi(argv[7]);
		process_wsarb m = process_wsarb();
		m.simply();
		m.print_output();
	}
	return 0;
